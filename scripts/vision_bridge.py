...
def analyze_image(image_path, question=None, verbose=False, context=None):
    """Send image to the vision model and return analysis text.

    Args:
        context: Optional neutral system-level guidance generated by the text
                 model (domain + observation focus). Injected as the `system`
                 field so the vision model knows *what* to look for without
                 being biased toward a specific expected outcome.

                 Format: "[Domain, ≤10 words]. Observe: [noun1], [noun2], [noun3]."
                 Max length: 60 words.
                 Forbidden: expect/want/hope to see, should have, I think/believe,
                            期望/想要/希望看到, 应该有, 我认为/我觉得.
    """
    if not API_KEY:
        return {
            "error": (
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python vision_bridge.py <image_path> [question|--brief] [--verbose] [--domain CONTEXT] [--context CONTEXT] [--region cn|sgp]")
        print()
    ...
    result = analyze_image(image_path, question, verbose, context=context_override)
    if "error" in result:
